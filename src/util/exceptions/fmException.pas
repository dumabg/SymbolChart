...
uses
  ...
  JclSysUtils,  JclUnitVersioning, JclUnitVersioningProviders, JclDebug, fmBase;

const
  UM_CREATEDETAILS = WM_USER + $100;

type
  TExceptionDialog = class(TfBase)
    OkBtn: TButton;
    DetailsBtn: TButton;
    BevelDetails: TBevel;
    DetailsMemo: TMemo;
    ...
    procedure bCerrarAppClick(Sender: TObject);
  private
    private
...
uses
  ...
  fmExceptionEnviando, dmInternalServer, UserServerCalls, dmConfiguracion,
  UtilException, uExceptionsManager;
...
procedure TExceptionDialog.bCerrarAppClick(Sender: TObject);
begin
  inherited;
  if Application.MessageBox('Ha pulsado el botón de Cerrar la aplicación, por lo que se intentará cerrar la aplicación inmediatamente.'
    + #13#10#13#10 +
    '¿Desea realmente cerrar la aplicación?',
    'Cerrar la aplicación', MB_YESNO + MB_ICONQUESTION) = IDYES then
...
//      StackList.AddToStrings(DetailsMemo.Lines, False, False, False, False);
//      NextDetailBlock;
      sl2 := GetStackTrace;
      try
        DetailsMemo.Lines.AddStrings(sl2);
      finally
        sl2.Free;
      end;
      NextDetailBlock;      
...
begin
  if Assigned(E) then
    if ExceptionShowing then begin
    end
    else begin
...
begin
  ...
  if Assigned(E) then
    exceptionManager.OnException(nil, E);
{    if ExceptionShowing then
      Application.ShowException(E)
    else
    begin
...
begin
  DrawIcon(Canvas.Handle, Memo2.Left - GetSystemMetrics(SM_CXICON) - 15,
    Memo2.Top, LoadIcon(0, IDI_ERROR));
end;
  ...
  try
    ...
    try
      ...
      try
        ...
        try
          bugServerCall.Call(Configuracion.Sistema.GUID, ReportAsText);
        finally
          bugServerCall.Free;
        end;
      finally
      ...
      ShowMessage('Gracias por su colaboración');
      Close;
    except on E:Exception do
      ShowMessage('No se ha podido enviar el informe de error.' + #13 + #13 +
        E.Message);
    end;
  finally
...
begin
  ...
  try
    ...
    begin
      DetailsMemo.Lines.Add(Configuracion.Sistema.Version);
      ...
      if E is Exception then
        DetailsMemo.Lines.Add(AdjustLineBreaks(StrEnsureSuffix('.', Exception(E).Message)))
      else
        DetailsMemo.Lines.Add(AdjustLineBreaks(StrEnsureSuffix('.', E.ClassName)));
      UpdateTextMemoScrollbars;
      NextDetailBlock;
