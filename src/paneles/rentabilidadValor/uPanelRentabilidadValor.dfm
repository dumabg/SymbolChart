inherited frPanelRentabilidadValor: TfrPanelRentabilidadValor
  Width = 596
  Height = 66
  ParentBackground = False
  ParentColor = False
  ExplicitWidth = 596
  ExplicitHeight = 66
  object ToolWindowRentabilidadValor: TSpTBXToolWindow
    Left = 0
    Top = 0
    Width = 591
    Height = 63
    Hint = 'Panel Rentabilidad del valor'
    Caption = 'Rentabilidad del valor'
    Color = 16054521
    CloseButtonWhenDocked = True
    DockPos = 423
    Resizable = False
    TabOrder = 0
    ClientAreaHeight = 63
    ClientAreaWidth = 591
    object pFondo: TPanel
      Left = 0
      Top = 0
      Width = 591
      Height = 63
      Align = alClient
      BevelOuter = bvNone
      Color = 16776436
      ParentBackground = False
      TabOrder = 0
      object Bevel5: TBevel
        Left = 249
        Top = 6
        Width = 7
        Height = 50
        Shape = bsLeftLine
      end
      object Image6: TImage
        Left = 263
        Top = 7
        Width = 15
        Height = 15
        Hint = 'Posiciones cerradas'
        AutoSize = True
        ParentShowHint = False
        Picture.Data = {
          
          2C50F8B02820003B}
        ShowHint = True
        Transparent = True
      end
      object Image7: TImage
        Left = 5
        Top = 7
        Width = 15
        Height = 15
        Hint = 'Posiciones abiertas'
        AutoSize = True
        ParentShowHint = False
        Picture.Data = {
          
          9123A60D183B8100003B}
        ShowHint = True
        Transparent = True
      end
      object Label27: TLabel
        Left = 433
        Top = 7
        Width = 74
        Height = 13
        AutoSize = False
        Caption = 'te'#243'rico cerrado'
      end
      object Label29: TLabel
        Left = 25
        Top = 7
        Width = 114
        Height = 13
        AutoSize = False
        Caption = 'Ultimo posicionamiento'
      end
      object Label30: TLabel
        Left = 171
        Top = 7
        Width = 71
        Height = 13
        AutoSize = False
        Caption = 'te'#243'rico abierto'
      end
      object Label40: TLabel
        Left = 159
        Top = 25
        Width = 45
        Height = 13
        Caption = 'resultado'
      end
      object Label45: TLabel
        Left = 286
        Top = 7
        Width = 114
        Height = 13
        AutoSize = False
        Caption = 'Ultimo posicionamiento'
      end
      object Label48: TLabel
        Left = 534
        Top = 25
        Width = 45
        Height = 13
        Caption = 'resultado'
      end
      object Label50: TLabel
        Left = 406
        Top = 25
        Width = 50
        Height = 13
        Caption = 'fecha final'
      end
      object Label53: TLabel
        Left = 19
        Top = 25
        Width = 53
        Height = 13
        Caption = 'fecha inicio'
      end
      object Label54: TLabel
        Left = 107
        Top = 25
        Width = 22
        Height = 13
        Caption = 'nivel'
      end
      object Label56: TLabel
        Left = 268
        Top = 25
        Width = 53
        Height = 13
        Caption = 'fecha inicio'
      end
      object Label57: TLabel
        Left = 339
        Top = 25
        Width = 48
        Height = 13
        Caption = 'nivel inicio'
      end
      object Label59: TLabel
        Left = 471
        Top = 25
        Width = 45
        Height = 13
        Caption = 'nivel final'
      end
      object RentabilidadAbierta: TDBText
        Left = 160
        Top = 41
        Width = 42
        Height = 17
        Hint = 'Resultado del '#250'ltimo posicionamiento te'#243'rico abierto'
        Alignment = taCenter
        DataField = 'RENTABILIDAD_ABIERTA'
        DataSource = RentabilidadValor.dsCotizacionEstado
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object RentabilidadAbiertaFecha: TDBText
        Left = 14
        Top = 41
        Width = 65
        Height = 17
        Cursor = crHandPoint
        Hint = 'Fecha de inicio del '#250'ltimo posicionamiento te'#243'rico abierto'
        Alignment = taCenter
        DataField = 'FECHA_INICIO_ABIERTA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = IrAFecha
      end
      object RentabilidadAbiertaNivel: TDBText
        Left = 89
        Top = 41
        Width = 59
        Height = 17
        Hint = 'Nivel de inicio del '#250'ltimo posicionamiento te'#243'rico abierto'
        Alignment = taCenter
        DataField = 'NIVEL_ABIERTA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object RentabilidadCerrado: TDBText
        Left = 527
        Top = 41
        Width = 57
        Height = 17
        Hint = 'Resultado del '#250'ltimo posicionamiento te'#243'rico cerrado'
        Alignment = taCenter
        DataField = 'RENTABILIDAD_CERRADA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object RentabilidadCerradoFechaFin: TDBText
        Left = 402
        Top = 41
        Width = 56
        Height = 17
        Cursor = crHandPoint
        Hint = 'Fecha final del '#250'ltimo posicionamiento te'#243'rico cerrado'
        Alignment = taCenter
        DataField = 'FECHA_INICIO_ABIERTA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = IrAFecha
      end
      object RentabilidadCerradoFechaIni: TDBText
        Left = 262
        Top = 41
        Width = 65
        Height = 17
        Cursor = crHandPoint
        Hint = 'Fecha de inicio del '#250'ltimo posicionamiento te'#243'rico cerrado'
        Alignment = taCenter
        DataField = 'FECHA_INICIO_CERRADA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = IrAFecha
      end
      object RentabilidadCerradoNivelFin: TDBText
        Left = 462
        Top = 41
        Width = 59
        Height = 17
        Hint = 'Nivel final del '#250'ltimo posicionamiento te'#243'rico cerrado'
        Alignment = taCenter
        DataField = 'NIVEL_ABIERTA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object RentabilidadCerradoNivelIni: TDBText
        Left = 333
        Top = 41
        Width = 61
        Height = 17
        Hint = 'Nivel de inicio del '#250'ltimo posicionamiento te'#243'rico cerrado'
        Alignment = taCenter
        DataField = 'NIVEL_CERRADA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object TipoPosAbierto: TDBText
        Left = 135
        Top = 7
        Width = 36
        Height = 17
        Hint = 'Tipo del '#250'ltimo posicionamiento te'#243'rico abierto'
        Alignment = taCenter
        DataField = 'TIPO_ABIERTA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object TipoPosCerrado: TDBText
        Left = 395
        Top = 7
        Width = 37
        Height = 17
        Hint = 'Tipo del '#250'ltimo posicionamiento te'#243'rico cerrado'
        Alignment = taCenter
        DataField = 'TIPO_CERRADA'
        DataSource = dsCotizacionRentabilidad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
      end
      object Image17: TImage
        Left = 262
        Top = 38
        Width = 5
        Height = 5
        AutoSize = True
        Picture.Data = {
          
          FF00000000FFFFFFFFFFFFFFFFFF00000000}
        OnClick = IrAFecha
      end
      object Image8: TImage
        Left = 400
        Top = 41
        Width = 5
        Height = 5
        AutoSize = True
        Picture.Data = {
          
          FF00000000FFFFFFFFFFFFFFFFFF00000000}
        OnClick = IrAFecha
      end
      object Image18: TImage
        Left = 13
        Top = 38
        Width = 5
        Height = 5
        AutoSize = True
        Picture.Data = {
          
          FF00000000FFFFFFFFFFFFFFFFFF00000000}
        OnClick = IrAFecha
      end
    end
  end
  object dsCotizacionRentabilidad: TDataSource
    DataSet = RentabilidadValor.CotizacionRentabilidad
    Left = 72
    Top = 8
  end
  object ApplicationEvents: TApplicationEvents
    OnShowHint = ApplicationEventsShowHint
    Left = 224
    Top = 16
  end
end
